...
const Cart = () => {
  ...
  try {
    for (const item of cartItems) {
      ...
      // ---------------------------------------------------------
      // 1) ORIGINAL PRODUCT UPDATE API (multipart/form-data)
      // ---------------------------------------------------------
      await axios
        .put(`http://localhost:8080/api/product/${item.id}`, cartProduct, {
          headers: {
      ...
      // ---------------------------------------------------------
      // 2) NEW STOCK UPDATE API CALL (PATCH)
      // ---------------------------------------------------------
      const buyStock = item.quantity; // quantity purchased

      await axios
        .patch(
          `http://localhost:8080/api/product/${item.id}/stock`,
          buyStock,
          {
            headers: {
    ...
    const emailPayload = {
      to: 'kartarsingh.gothwal22@vit.edu',                     // Customer email
      customerName: 'kartar singh',            // Customer name
      orderId: Date.now(),                // simple order ID (timestamp) – replace with actual
    ...
    await axios
      .post("http://localhost:8080/api/email/send-order-email", emailPayload)
      .then(() => {
